---
title: "Statistical Analysis - Binary Logistic Regression - Football Salaries Dataset"
subtitle: "PSI CA Part Three - Logistic Regression" 
...
-   Variables: Age(Continuous), Transformed Salary (Continuous)
...
Since both variables are continuous and Salary has been log-transformed to meet normality assumptions, Pearson’s correlation was used to test for the relationship between Age and Salary.
...
games_howell_result <- filtered_data %>%
  games_howell_test(log_salary ~ Based)

# Print the results
print(games_howell_result, n = 100)
```
...
-   *Conclusion*: A chi-square test of independence revealed a statistically significant association between EU nationality and the league/nation (Based) where the club is located (𝜒2=21,868, df=805, p\<0.001). Fisher’s Exact Test with simulated p-values was used as a robustness check and produced a consistent result. This provides evidence to reject the null hypothesis in favour of the alternative hypothesis. The observed association indicates that players’ nationalities (EU or non-EU) are significantly related to the leagues or nations in which they play, likely reflecting regional labour market structures, work permit regulations, and eligibility requirements across different football leagues.
...
This statistical analysis explores whether a range of factors predict the likelihood of a football player being in a top 5 football league. Specifically, the analysis addresses the following research question:\

**Do factors like a player's age, reputation, and salary, controlling for their nationality and historical club affiliations, predict the likelihood of a player being in a top 5 football league?**

Additionally, the analysis seeks to examine whether the predictive relationship between these factors and a player's likelihood of being in a top 5 league differs by nationality.

Binary logistic regression is used to investigate these questions, with two models built. The first model addresses the hypothesis that a combination of factors such as age, reputation, and salary can significantly predict whether a player is in a top 5 football league. The second model explores whether the predictive relationship between these factors and league status varies depending on a player’s nationality.

The dataset used for this analysis was collected from 35,478 football players, containing data on a variety of factors such as demographics, football-related metrics, and player background. This dataset is made available from publicly accessible data on player profiles and is referenced from recent football analysis studies.

Section 2 will describe the dataset used and the steps taken to prepare it for the logistic regression models. Section 3 will present the fitting of the models, diagnostic checks, and the interpretation of the results. Section 4 will compare the two models built and discuss their relative usefulness. Finally, conclusions from the analysis will be drawn in Section 5.

A significance threshold of 0.05 will be used throughout the analysis, and effect sizes will be reported according to Cohen's conventions (Cohen, 1988).
...
The dataset used in this analysis consists of 35,478 rows and 14 columns, capturing data on football players across various leagues. The dataset was designed to explore the factors affecting players’ likelihood of being in a top football league.

The key variables in this analysis are age, reputation, and salary, which are considered predictors of a player's likelihood of being in a top 5 football league. The outcome variable in this dataset, Is_top_5_League, is binary, coded as (1 = in a top 5 league, 0 = not in a top 5 league).

In addition to these key predictors, the dataset includes several demographic, performance, and career-related factors. The EU_National variable indicates whether a player is from an EU country, and Is_top_prev_club reflects whether the player has played for a top club previously. Other variables include salary, reputation, and additional career metrics like the number of international appearances (Caps) and the number of games played for their club (Apps).

##### 9.2.1 Demographic and Background Factors

The demographic and background factors in this analysis include the following variables:

-   **Age**: A continuous variable representing the age of each player. Age is a crucial demographic factor that may influence a player's likelihood of playing in top leagues, with younger players often being valued for their potential.
-   **EU_National**: A binary variable indicating whether a player is from an EU country (1 = Yes, 0 = No). This is an important factor because players from EU countries may have different opportunities or restrictions related to transfers and league placements.
-   **Is_top_prev_club**: A binary variable indicating whether a player has previously played for a top football club (1 = Yes, 0 = No). This variable reflects a player's career background, as players with experience in top clubs may be more likely to transfer to top leagues.
-   **Nationality**: A variable representing the player's country of origin. This variable can help examine whether a player's nationality influences their likelihood of being in a top league.

##### 9.2.2 Performance and Career Metrics

The performance and career metrics in this analysis include the following variables:

-   **Salary**: A continuous variable representing the salary of the player in USD. Salary often correlates with a player's market value and may indicate their likelihood of being in a top football league, as higher-paid players are more likely to play in leagues with significant financial resources.
-   **Reputation**: A continuous variable that measures the player's reputation within the football world, often related to their performance on the field, media presence, and popularity. A higher reputation may increase the likelihood of a player being in a top league.
-   **Caps**: A continuous variable representing the total number of international appearances (caps) a player has made. Players with more international experience may have higher chances of being recruited for top leagues.
-   **Apps**: A continuous variable reflecting the total number of games a player has played for their club. Similar to caps, this variable indicates the player’s professional experience, which is often considered when determining their worth for top leagues.

##### 9.2.3 Statistical Considerations

Descriptive statistics for the continuous variables (Age, Reputation, Salary, Caps, and Apps) were calculated, and the distribution of each variable was assessed for normality. The results from these tests are summarized below. The variables of interest were inspected for missing data, with no significant issues found. Additionally, the distribution of the variables was checked using visual methods such as histograms and QQ plots, and further analyzed using skewness and kurtosis measures. The normality of these variables was considered crucial for the logistic regression analysis, and any necessary transformations were applied to ensure compliance with assumptions for parametric tests.

The key statistics for each of these variables are shown in Table 1 below.
...
| **Is_top_5_League**  | Binary (0 = No, 1 = Yes) | Nominal    |
| **Age**              | Continuous (years)       | Continuous |
| **Reputation**       | Continuous (score)       | Continuous |
| **log_salary**       | Continuous (USD)         | Continuous |
| **sqrt_caps**        | Continuous (games)       | Continuous |
| **Apps**             | Continuous (games)       | Continuous |
| **EU_National**      | Binary (1 = Yes, 0 = No) | Nominal    |
| **Is_top_prev_club** | Binary (1 = Yes, 0 = No) | Nominal    |

##### 9.2.4 Missing Data

The data for each of the concepts of interest was inspected to determine whether the level of missing data could represent a source of bias. Following the guidelines provided by Tabachnick and Fidell (2016), it was found that neither the proportion nor pattern of missing data presented a significant issue. All missing data were removed before the analysis, and it was confirmed that the missing data did not significantly impact the results.
...
summary(data$Age)
summary(data$Is_top_5_League)
summary(data$Is_top_prev_club)
summary(data$EU_National)
summary(data$sqrt_caps)
summary(data$Apps)
...
#### 9.3 Predicting Likelihood of Playing in a Top 5 Football League: The Role of Age, Salary, and Reputation

This section outlines the binary logistic regression model developed to assess the extent to which player characteristics— age, salary, and reputation — along with contextual factors such as being an EU national, predict the likelihood of a player joining one of the Top 5 football leagues. The model’s fit and usefulness are evaluated, followed by diagnostic checks to ensure reliability and validity. Finally, the contribution of each predictor to the model is examined, and the implications for player career trajectories are discussed.

##### 9.3.2 Model Fitting

A logistic regression model was developed using four predictors: Age, Salary, Reputation, and EU National status. The outcome variable, Is_top_5_League, was coded as a binary nominal variable (0 = Not in a Top 5 League, 1 = In a Top 5 League). The model was fitted using a binomial family with a logit link.

The model equation is given by:

</p>

<p>$$ \text{logit}(P(Y=1)) = \beta_0 + \beta_1X_1 + \beta_2X_2 + \dots + \beta_nX_n $$</p>

```{r, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
# Build the logistic regression model

#model <- glm(Is_top_5_League ~ Age + log_salary + Reputation + EU_National + Is_top_prev_club,
#             data = data, 
#             family = binomial)    ---> poor fit according to the Hosmer-Lemeshow test (p-value<0.05) 

#model <- glm(Is_top_5_League ~ Age + log_salary + Reputation + EU_National,
#             data = data, 
#             family = binomial)    ---> poor fit according to the Hosmer-Lemeshow test (p-value<0.05) 

# Add interaction terms between Age and log_salary
model <- glm(Is_top_5_League ~ Age * log_salary + Reputation + EU_National, 
             data = data,
             family = binomial)
...
TThe fitted model was statistically significant, as indicated by a reduction in deviance compared to the null model (Null Deviance = 20225, Residual Deviance = 11372; ΔDeviance = 7219, p \< 0.001). This suggests that the predictors significantly improve the model’s ability to classify players into the Top 5 leagues.

The coefficients and their significance levels are summarized below:

|  |  |  |  |  |  |
|------------|------------|------------|------------|------------|------------|
| **Predictor** | **Estimate** | **Std. Error** | **z-value** | **p-value** | **Odds Ratio** |
| **Intercept** | 21.44 | 1.23 | 17.43 | \< 2e-16 | \- |
| **Age** | -1.645 | 0.056 | -29.47 | \< 2e-16 | 0.192 |
| **log_salary** | -1.782 | 0.101 | -17.57 | \< 2e-16 | 0.169 |
| **Reputatio** | 0.000794 | 0.00005 | 16 | \< 2e-16 | 1.001 |
| **EU_National** | 0.944 | 0.056 | 16.72 | \< 2e-16 | 2.564 |

##### 9.3.3 Model Diagnostics

No substantial deviation from linearity was detected, as assessed by the Hosmer-Lemeshow test (χ²(8, N=35,478) = 14.33, p = 0.0735), suggesting a good fit of the model to the data. The p-value (0.0735) is above the threshold (0.05), which means there is no significant lack of fit in the model.

Collinearity diagnostics revealed no significant multicollinearity issues among the predictors, with variance inflation factors (VIF) well below the commonly accepted threshold of 5:

-   Age: VIF = 1.14, tolerance = 0.88
-   log_salary: VIF = 1.54, tolerance = 0.65
-   Reputation: VIF = 1.56, tolerance = 0.64
-   EU National: VIF = 1.01, tolerance = 0.99

These results suggest that the predictors in the model are independently contributing to the outcome variable without significant overlap. However, the interaction term between Age and log_salary exhibited elevated multicollinearity, with a VIF of 202.34, indicating potential instability in the estimation of coefficients involving this interaction.

The model's pseudo R-squared values provide insight into its explanatory power. McFadden's R² = 0.44, Cox and Snell R² = 0.22, and Nagelkerke R² = 0.51 indicate that the model explains between 22% and 51% of the variability in the likelihood of playing in a Top 5 League, which reflects moderate to strong explanatory power. This suggests that other unmeasured factors may also contribute to player selection into Top 5 leagues.
...
influential <- cooks.distance(model)

# Deviance Residuals Summary
residuals_summary <- summary(residuals(model, type = "deviance"))
cat("Deviance Residuals Summary:\n")
print(residuals_summary)

# Cook's Distance
cooks_d <- cooks.distance(model)
n <- nrow(data)  # Total number of observations
cooks_threshold <- 4 / n  # Threshold for influential observations

# Influential Observations
influential_obs <- which(cooks_d > cooks_threshold)
cat("Number of Influential Observations:", length(influential_obs), "\n")
cat("Top 5 Influential Observations:\n")
print(sort(cooks_d, decreasing = TRUE)[1:5])

# Pseudo R-squared values
library(rcompanion)
pseudo_r2 <- nagelkerke(model)

# Print diagnostics
list(hoslem_test = hoslem_test, vif_values = vif_values, pseudo_r2 = pseudo_r2)

```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Deviance Residuals Histogram
hist(residuals(model, type = "deviance"),
...
cooks_d <- cooks.distance(model)
plot(cooks_d, type = "h", col = ifelse(cooks_d > (4/nrow(data)), "red", "blue"),
...
The logistic regression model indicated several significant predictors for the likelihood of playing in a Top 5 League. The intercept was positive and highly significant (B = 21.44, z = 17.43, p \< 0.001), representing the baseline odds when all predictors are set to zero.

**Predictor Results:**

-   Age: A significant negative predictor (B = -1.645, z = -29.47, p \< 0.001, OR = 0.192). This suggests that older players are less likely to join a Top 5 League. For each additional year of age, the odds of joining a Top 5 League decrease by approximately 80.8%.
-   Salary: A significant negative predictor (B = -1.782, z = -17.57, p \< 0.001, OR = 0.169). Higher salaries are associated with a lower likelihood of joining a Top 5 League, with the odds decreasing by approximately 83.1% for each unit increase in salary (log-transformed).
-   Reputation: A significant positive predictor (B = 0.000794, z = 16.00, p \< 0.001, OR = 1.001). Each additional unit of reputation increases the odds of joining a Top 5 League by approximately 0.1%.
-   EU National: A significant positive predictor (B = 0.944, z = 16.72, p \< 0.001, OR = 2.564). EU nationals are more than 2.5 times more likely to play in a Top 5 League compared to non-EU players.

**Predictive Performance:**

The area under the ROC curve (AUC) was 0.9317, indicating excellent discrimination ability of the model. The model's sensitivity was 0.4058, suggesting that 40.6% of the actual positive cases (players in a Top 5 League) were correctly identified. On the other hand, the specificity was 0.9871, meaning that 98.7% of the actual negative cases (players not in a Top 5 League) were correctly identified. This indicates that the model is particularly good at identifying players who are not in a Top 5 League, and while the sensitivity is relatively moderate, the model still does well in identifying positive cases compared to random guessing.

```{r, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
# ROC Curve and AUC calculation
library(pROC)

# Generate predicted probabilities for ROC
predicted_probs <- predict(model, type = "response")

# Create ROC curve
roc_curve <- roc(data$Is_top_5_League, predicted_probs)
...
# Output results
list(AUC = auc_value, Sensitivity = sensitivity, Specificity = specificity)

# Plot the ROC curve
plot(roc_curve, main = "ROC Curve", col = "blue")
text(0.5, 0.3, paste("AUC = ", round(roc_curve$auc, 3)), col = "blue")

```

#### 9.4 Conclusion

This binary logistic regression analysis revealed significant predictors of the likelihood of a football player joining one of the Top 5 leagues. Specifically, age, salary, reputation, and EU national status were found to have a meaningful influence on the outcome variable, with several key findings emerging from the model.

The significant negative coefficients for both age (B = -1.645, OR = 0.192) and salary (B = -1.782, OR = 0.169) indicate that older players and those with higher salaries are less likely to join a Top 5 league. In contrast, the reputation of a player (B = 0.000794, OR = 1.001) and EU nationality (B = 0.944, OR = 2.564) were positive predictors, with players from the EU being significantly more likely to play in a Top 5 league, and higher reputation contributing to the likelihood of joining such leagues. These findings suggest that player experience and market value have a substantial role in determining the selection for elite football leagues, while nationality can also play a considerable part.

The model demonstrated strong statistical significance (p \< 0.001 for all predictors) and moderate explanatory power. McFadden's R² of 0.44, Cox and Snell R² of 0.22, and Nagelkerke R² of 0.51 indicate that the model explains between 22% and 51% of the variability in the likelihood of a player joining a Top 5 league. While the model's explanatory power is moderate, this highlights the influence of other unmeasured factors that may also play a role in predicting recruitment into these elite leagues.

Diagnostic checks supported the model’s validity. The Hosmer-Lemeshow test (χ²(8, N=35,478) = 14.33, p = 0.0735) indicated a good fit, with no substantial deviations from linearity. Variance Inflation Factors (VIF) for most predictors were within acceptable limits, though the interaction between age and log_salary showed elevated multicollinearity, which may necessitate caution in interpreting the coefficients of this interaction term.

The model also demonstrated strong predictive ability, with an AUC of 0.9317, indicating excellent discrimination. However, while the model's specificity was high (98.7%), its sensitivity was more moderate (40.6%). This trade-off between sensitivity and specificity suggests that while the model is highly effective at identifying players unlikely to join a Top 5 league, it could benefit from additional refinements to improve its predictive accuracy for those who will.

To illustrate the model's usefulness, sample predictions based on average values and alternate scenarios reinforced the significant role of predictors like reputation and EU nationality. For instance, increasing a player’s reputation by one standard deviation while holding other variables constant substantially increased their predicted probability of joining a Top 5 league.

In conclusion, the model has demonstrated that player characteristics such as age, salary, reputation, and nationality are key determinants in predicting whether a player will join one of the Top 5 football leagues. However, given the moderate explanatory power and sensitivity of the model, future research should explore additional predictors and interactions to improve its predictive performance. For instance, incorporating factors like playing style, injury history, or the influence of non-European nationalities could provide a more nuanced understanding of recruitment dynamics. Furthermore, employing advanced modeling techniques, such as machine learning algorithms, might improve both sensitivity and specificity, leading to better decision-making for player recruitment in top-tier football leagues.
...
Field, A., Field, Z., & Miles, J. (2012). Discovering Statistics Using IBM SPSS Statistics. Sage Publications Ltd.

Pallant, J. (2020). SPSS Survival Manual: A Step-by-Step Guide to Data Analysis Using IBM SPSS (7th ed.). Taylor and Francis.

Watson, D., Clark, L. A., & Tellegen, A. (1988). Development and validation of brief measures of positive and negative affect: The PANAS scales. Journal of Personality and Social Psychology, 54(6), 1063–1070.

Tabachnik, B. G., & Fidell, L. S. (2013). Using Multivariate Statistics (6th ed.). Pearson Education.
